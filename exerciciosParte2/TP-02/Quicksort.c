...
int comparar(Restaurante *a, Restaurante *b, long *comparacoes) {
    int resp = 0;
...
void trocar(Restaurante **a, Restaurante **b, long *movimentacoes) {
    Restaurante *tmp = *a;
    *a = *b;
    *b = tmp;
...
void quicksort(Restaurante **array, int esq, int dir, long *comparacoes, long *movimentacoes) {
    int i = esq;
    int j = dir;
    ...
    while (i <= j) {
        ...
        if (i <= j) {
            trocar(&array[i], &array[j], movimentacoes);
            i++;
            j--;
        }
    }
...
void escrever_log(long comparacoes, long movimentacoes, double tempo) {
    FILE *arquivo = fopen(MATRICULA "_quicksort.txt", "w");
