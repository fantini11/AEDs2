 ...
 * Mantem apenas os 10 menores nomes nas primeiras posicoes.
 */
void insercao_parcial(Restaurante **array, int n, long *comparacoes, long *movimentacoes) {
    int k = 10;
    ...
    for (int i = k; i < n; i++) {
        *comparacoes = *comparacoes + 1;
        ...
        if (strcmp(array[i]->nome, array[k - 1]->nome) < 0) {
            Restaurante *tmp = array[i];
            *movimentacoes = *movimentacoes + 1;
...
void escrever_log(long comparacoes, long movimentacoes, double tempo) {
    FILE *arquivo = fopen(MATRICULA "_insercao_parcial.txt", "w");
...
int main() {
    ...
    insercao_parcial(selecionados, n, &comparacoes, &movimentacoes);
